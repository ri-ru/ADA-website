...
export const fontStack = '"TeX Gyre Adventor", "Tex Gyre Adventor", "Helvetica Neue", Arial, sans-serif';
...
export const categories = ["Gaming","Entertainment","Music","News & Politics","People & Blogs","Sports","Howto & Style","Education","Science & Technology","Film & Animation","Autos & Vehicles","Comedy","Travel & Events","Nonprofits & Activism","Pets & Animals"];
export const ECHARTS_ORDER = ["Autos & Vehicles","Comedy","Education","Entertainment","Film & Animation","Gaming","Howto & Style","Music","News & Politics","Nonprofits & Activism","People & Blogs","Pets & Animals","Science & Technology","Sports","Travel & Events"];
...
export const categoryColors = categories.reduce((acc, cat) => {
...
export const years = dataRows.map((d) => d.year);
export const urlDataByYear = {
...
export function hexToRgba(hex, alpha = 1) {
  ...
  const bigint = parseInt(val.length === 3 ? val.split('').map((c) => c + c).join('') : val, 16);
...
export function colorWithAlpha(color, alpha = 0.22) {
...
export function makeGradient(ctx, color, height = 300) {
  ...
  grad.addColorStop(1, hexToRgba(color, 0.04));
  return grad;
}

export function buildSeriesStyles(catColors = categoryColors) {
  return {
    URLs: { color: catColors['Gaming'] || '#38bdf8', dashed: false },
    Monetization: { color: '#22c55e', dashed: true },
    Content: { color: '#38bdf8', dashed: true },
    Social: { color: chartTheme.color?.[1] || '#f97316', dashed: true },
  };
}

export function buildAggregateSeries(metric) {
  const key = metric.toLowerCase();
  const totalsAll = years.map((y) => dataRows.find((r) => r.year === y)).map((row) =>
    categories.reduce((s, c) => s + (row?.[c] || 0), 0)
  );
  const absSeries = key === 'urls'
    ? years.map((y) => categories.reduce((s, c) => s + ((urlDataByYear[y] || {})[c] || 0), 0))
    : years.map((y) => categories.reduce((s, c) => {
        const catYear = (specialtyData[c] || {})[y] || {};
        return s + (catYear[key] || 0);
      }, 0));
  const pctSeries = absSeries.map((v, i) => (totalsAll[i] ? (v / totalsAll[i]) * 100 : 0));
  return { absSeries, pctSeries, totalsAll };
}

export function buildCategorySeries(metric) {
  const key = metric.toLowerCase();
  return categories.map((cat) => {
    ...
    const abs = key === 'urls'
      ? years.map((y) => (urlDataByYear[y] || {})[cat] || 0)
      : years.map((y) => ((specialtyData[cat] || {})[y] || {})[key] || 0);
    const pct = abs.map((v, i) => (totals[i] ? (v / totals[i]) * 100 : 0));
    return { cat, abs, totals, pct };
  });
}

export function buildDatasets(cat, seriesVisible, seriesStyles = buildSeriesStyles()) {
  const urlsAbs = years.map((y) => (urlDataByYear[y] || {})[cat] || 0);
  const totals = years.map((y) => {
    const row = dataRows.find((r) => r.year === y);
    return row ? row[cat] || 0 : 0;
  });
  const linkAbs = linkTypes.map((type) => years.map((y) => ((specialtyData[cat] || {})[y] || {})[type] || 0));
...
export function getLayoutDims(count) {
