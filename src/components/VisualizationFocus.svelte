<script>
  ...
  const categories = [
    "Gaming","Entertainment","Music","News & Politics","People & Blogs","Sports","Howto & Style","Education","Science & Technology","Film & Animation","Autos & Vehicles","Comedy","Travel & Events","Nonprofits & Activism","Pets & Animals",
  ];

  const categoryEmojis = {
    "Autos & Vehicles":"🚗","Comedy":"😂","Education":"📘","Entertainment":"🎬","Film & Animation":"🎞️","Gaming":"🎮","Howto & Style":"🧵","Music":"🎵","News & Politics":"📰","Nonprofits & Activism":"🤝","People & Blogs":"👥","Pets & Animals":"🐾","Science & Technology":"🔬","Sports":"🏅","Travel & Events":"✈️"
  };

  const palette = ["#0ea5e9","#f97316","#22c55e","#eab308","#a855f7","#fb7185","#f43f5e","#14b8a6","#6366f1","#ec4899","#84cc16","#06b6d4","#ef4444","#8b5cf6","#facc15"];
  ...
  // Computed colors based on active category
  let activeCategoryColor = $derived(categoryColors[activeCategory] || "#38bdf8");
  const monetColor = linkColors["monetization"] || "#22d3ee";
  const contentColor = linkColors["content"] || "#a855f7";
  const socialColor = linkColors["social"] || "#f97316";
  ...
  function urlShare(year, cat, urlsVal) {
    const row = dataRows.find(r=>r.year===year);
    if (!row) return 0;
    const total = row[cat] || 0;
    if (!total) return 0;
    return (urlsVal/total)*100;
  }

  function buildDatasets(cat) {
    const urlsSeriesAbs = years.map(y => (urlDataByYear[y]||{})[cat] || 0);
    const baseSeries = years.map(y => {
      const row = dataRows.find(r=>r.year===y);
      return row ? (row[cat]||0) : 0;
    });
    const catSpec = specialtyData[cat] || {};
    const linkSeriesAbs = linkTypes.map(type => years.map(y => (catSpec[y]||{})[type] || 0));

    const urlsSeriesPct = urlsSeriesAbs.map((val, idx) => {
      const total = baseSeries[idx] || 0;
      return total ? (val / total) * 100 : 0;
    });
    const linkSeriesPct = linkSeriesAbs.map(series =>
      series.map((val, idx) => {
        const total = baseSeries[idx] || 0;
        return total ? (val / total) * 100 : 0;
      })
    ...
    return { datasets, baseSeries, urlsSeriesAbs, linkSeriesAbs };
  }

  function renderChart() {
    if (focusChart) focusChart.destroy();
    const cat = activeCategory;
    const { datasets, baseSeries, urlsSeriesAbs, linkSeriesAbs } = buildDatasets(cat);
    const color = categoryColors[cat] || "#38bdf8";

    datasets.forEach(ds => {
      ds.hidden = !seriesVisible[ds.label];
    });
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: (ctx) => {
                ...
                const urlsVal = urlsSeriesAbs[ctx.dataIndex] || 0;
                const totalVal = baseSeries[ctx.dataIndex] || 0;
                const nice = (s) => s.charAt(0).toUpperCase() + s.slice(1);
                if (ctx.dataset.label === "URLs") {
                  const pct = urlShare(year, cat, urlsVal).toFixed(1);
                  return [`URLs: ${pct}% • ${formatMillions(urlsVal)} of ${formatMillions(totalVal)}`];
                }
                const typeKey = ctx.dataset.label.toLowerCase();
                const typeVal = (linkSeriesAbs[linkTypes.indexOf(typeKey)] || [])[ctx.dataIndex] || 0;
                const pctOfTotal = totalVal ? ((typeVal / totalVal) * 100).toFixed(1) : "0.0";
                const label = nice(ctx.dataset.label);
                return `${label}: ${pctOfTotal}% • ${formatMillions(typeVal)} of ${formatMillions(totalVal)}`;
              }
            }
          }
        }
  ...
  function toggleSeries(key) {
    const activeKeys = Object.keys(seriesVisible).filter(k => seriesVisible[k]);
    const isOnly = activeKeys.length === 1 && activeKeys[0] === key;
    if (isOnly) {
      Object.keys(seriesVisible).forEach(k => seriesVisible[k] = true);
    } else {
      Object.keys(seriesVisible).forEach(k => seriesVisible[k] = (k === key));
    }
    renderChart();
  }
...
<style>
  ...
  .legend-toggle.dashed .swatch { border-style: dashed; }
  .legend-toggle.inactive { opacity: 0.45; }

  canvas { width: 100% !important; height: 100% !important; flex: 1; }
</style>

<div class="viz-focus-wrap">
  <div class="toolbar">
    <label for="catSelect" style="font-size:12px;color:#94a3b8;">Select category:</label>
    <select id="catSelect" bind:value={activeCategory} onchange={() => setCategory(activeCategory)}>
  ...
  <div class="category-legend">
    {#each categories as cat}
      ...
      <button 
        ...
        style={isActive ? `border-color:${color};background:${color}22;color:#f8fafc;` : ""}
  ...
  <div class="chart-card">
    <div class="series-bar">
      <button 
        class="legend-toggle {seriesVisible.URLs ? '' : 'inactive'}" 
        style="color:{activeCategoryColor}"
        onclick={() => toggleSeries('URLs')}
      >
        <span class="swatch"></span> URLs
      </button>
      <button 
        class="legend-toggle dashed {seriesVisible.Monetization ? '' : 'inactive'}" 
        style="color:{monetColor}"
        onclick={() => toggleSeries('Monetization')}
      >
        <span class="swatch"></span> Monetization
      </button>
      <button 
        class="legend-toggle dashed {seriesVisible.Content ? '' : 'inactive'}" 
        style="color:{contentColor}"
        onclick={() => toggleSeries('Content')}
      >
        <span class="swatch"></span> Content
      </button>
      <button 
        class="legend-toggle dashed {seriesVisible.Social ? '' : 'inactive'}" 
        style="color:{socialColor}"
        onclick={() => toggleSeries('Social')}
      >
        <span class="swatch"></span> Social
      </button>
    </div>
